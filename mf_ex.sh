...
case "$1" in
    --linux)
        OUT_NAME=KalaGraphics-Linux

        LIB_NAME=libkalagraphics
        LIB_EXT=a
        LIB_ORIGIN=build/release-linux
        ;;
    --win-on-linux)
        OUT_NAME=KalaGraphics-Win-on-Linux

        LIB_NAME=kalagraphics-gnu
        LIB_EXT=lib
        LIB_ORIGIN=build/release-win-on-linux
        ;;
    --windows)
        OUT_NAME=KalaGraphics-Windows

        LIB_NAME=kalagraphics
        LIB_EXT=
